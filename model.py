...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(self.block_norm1(x)).view(shape)  # batch_size, n_patches +1, 3, n_heads, heads_dim
        qkv = qkv.permute(2, 0, 3, 1, 4)  # 3, batch_size, n_heads, n_patches +1, heads_dim
        q, k, v = qkv[0], qkv[1], qkv[2]  # each (batch_size, n_heads, n_patches +1, heads_dim)
        attn = q @ k.transpose(-2, -1) * self.scale  # (batch_size, n_heads, n_patches +1, n_patches +1)
        attn = self.attn_drop(attn.softmax(dim=-1))  # (batch_size, n_heads, n_patches +1, n_patches +1)
        attn_out = attn @ v  # (batch_size, n_heads, n_patches +1, head_dim)
        attn_out = attn_out.transpose(1, 2)  # (batch_size, n_patches + 1, n_heads, head_dim)
        attn_out = attn_out.flatten(2)  # (batch_size, n_patches + 1, dim)
        # для визуализации
        self.attn = attn

        att = self.block_drop(self.proj(attn_out))  # (batch_size, n_patches + 1, dim)
        x = x + att
        mlp = self.block_drop(self.mlp(self.block_norm2(x)))  # (batch_size, n_patches + 1, hidden_features)
        x = x + mlp
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((cls_token, x), dim=1)  # (batch_size, 1 + n_patches, dim)
        if self.pos_1d:
            x = x + self.pos_embed  # (batch_size, 1 + n_patches, dim)
        else:
        ...
        x = self.norm(x)  # (batch_size, 1 + n_patches, dim)
